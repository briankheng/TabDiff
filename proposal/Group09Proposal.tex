...
Passion Goh Wei Ling & \fillin{A0000000X} \\
\bottomrule
\end{tabular}
\vfill
...
For each of two network intrusion datasets we get a labelled training set and have to submit a fixed number of synthetic normal rows: 40,000 for NSL-KDD and 70,000 for UNSW-NB15. The organisers fit two fixed detectors on those rows, ECOD and an Isolation Forest (10 seeds of 500 trees), and score them by the area under the precision-recall curve (AUPRC) on hidden validation and test sets. The final score is the mean of the two AUPRCs, and the hidden sets contain attack types that never appear in training.

This makes the task a bit unusual. We never train a detector ourselves, and the rows we submit are, as far as the detectors know, the definition of normal traffic. A good submission has to cover the normal traffic in the hidden sets, so that it isn't flagged, while leaving the places where attacks live empty, so that attacks land in the tails. Copying the training distribution does the first job but not the second.

Our plan is to train a mixed-type diffusion model, TabDiff~\cite{shi2025tabdiff}, on all training rows with the label as an extra column, and then sample normal rows with a guidance term that pushes each sample away from what the model associates with attacks. Section~\ref{sec:eda} looks at where the attacks actually sit in the data, Section~\ref{sec:related} covers related work, and Section~\ref{sec:approach} describes the method, the evidence for it so far, and how we will evaluate it.

\section{Exploratory data analysis}
\label{sec:eda}

Table~\ref{tab:data} summarises the two training sets. Both are mixed-type tables with three categorical columns, and both have a little over twice as many normal rows as attacks. The numeric columns are hard to model. In NSL-KDD, 23 of the 38 numeric columns are zero in over 90\% of normal rows, and \col{src\_bytes} has a skewness of about 129 among normal rows. Eleven NSL-KDD columns and ten UNSW-NB15 columns take at most 20 distinct values (flags, TTLs and small counters), and two NSL-KDD columns, \col{num\_outbound\_cmds} and \col{is\_host\_login}, are constant. A generator that treats all of these as continuous will waste probability mass on values that never occur, such as a TTL of 45.
...
The briefing distinguishes anomalies with an odd value from anomalies that are a rare combination of ordinary values, so we checked which kind the training attacks are. For each attack row we asked whether any numeric value falls outside the range seen in normal rows, or whether any categorical value never appears in a normal row. In NSL-KDD, 32.5\% of attacks use a category that no normal row uses (41 of the 67 \col{service} values appear only in attacks), but only 2.0\% have an out-of-range number. The other 65.5\% sit inside the normal range on every column. UNSW-NB15 is far more extreme: 97.2\% of its attacks are inside the normal range everywhere.
...
Figure~\ref{fig:ks} ranks the numeric columns by the Kolmogorov-Smirnov (KS) distance between their normal and attack values. NSL-KDD has 13 columns above 0.5, led by the byte counts. UNSW-NB15 has three, all derived from TTLs, and these show how an attack can look unremarkable one column at a time. Every UNSW-NB15 attack has \col{sttl} equal to 254 (71.5\%) or 62 (28.5\%), and both values are common in normal traffic too, at 27.1\% and 7.0\% of normal rows. The attacks give themselves away through how often these values occur and what they occur together with, not through the values themselves.

This matters because of how the two detectors work. ECOD scores each column by its tail probability under the empirical distribution of the training rows and adds the scores up, so it only responds to odd values. Isolation Forest can isolate rare combinations with its random axis-parallel splits, but with scikit-learn's default settings each tree only sees 256 rows. Both are fitted on our submission. So on UNSW-NB15, the only way to raise an attack's ECOD score is to make values such as $\col{sttl}=254$ rarer in the synthetic data than they are in real normal traffic, and a faithful copy of the normal rows cannot do that. This fits the low scores on this dataset: the provided sample submission scores 0.383 on the public leaderboard for UNSW-NB15, against 0.899 for NSL-KDD.
...
Chandola et al.~\cite{chandola2009survey} survey the classical anomaly detection methods and Pang et al.~\cite{pang2021deep} the deep ones. The two detectors used here are fast, unsupervised methods for tabular data. Isolation Forest~\cite{liu2008iforest} builds random trees on small subsamples and scores a point by how few random splits it takes to isolate it, so it responds to low density without needing a distance metric. ECOD~\cite{li2022ecod} estimates the empirical cumulative distribution function of each feature and scores a point by its tail probabilities, summed over features. It has no parameters and is easy to interpret, but it ignores the dependence between features, which is exactly the weakness Section~\ref{sec:eda} points at.

ADBench~\cite{han2022adbench} compared 30 detectors on 57 datasets and found that no unsupervised method was significantly better than the others overall. Which one wins depends on whether its assumptions match the type of anomaly. In this project the detectors are fixed, so we can't pick a method to suit the anomalies. What we control is the training data, the other half of that match.

\subsection{Tabular data synthesis}

The simplest augmentation is interpolation. SMOTE~\cite{chawla2002smote} creates new rows on the line between a row and one of its nearest neighbours. It is cheap, but it only fills in between existing rows and has no natural way to handle categorical columns. CTGAN and TVAE~\cite{xu2019ctgan} model mixed-type tables with a GAN and a variational autoencoder, using mode-specific normalisation for multi-modal numeric columns and a conditional generator so that rare categories still get sampled. TabDDPM~\cite{kotelnikov2023tabddpm} brought diffusion models to tables, with Gaussian diffusion for numeric columns and multinomial diffusion for categorical ones. TabSyn~\cite{zhang2024tabsyn} instead runs score-based diffusion in the latent space of a VAE. TabDiff~\cite{shi2025tabdiff} trains a single continuous-time diffusion over both types, with masking diffusion for categorical columns and a learnable noise schedule for every column, and reports better fidelity than these earlier models on the benchmarks it tests.

These papers judge synthetic data mainly by fidelity, meaning how closely its marginals and correlations match the real data, and by utility: train a model on synthetic data and test it on real data, a protocol usually traced back to Esteban et al.~\cite{esteban2017tstr}. Our task is a utility test where the model is fixed to two unsupervised detectors. That changes the goal, since the most faithful submission is not necessarily the most useful one.

\subsection{Guidance in diffusion models}

A conditional diffusion model can be steered while sampling. Classifier guidance~\cite{dhariwal2021beat} adds the gradient of a separately trained classifier to the model's score. Classifier-free guidance~\cite{ho2022cfg} drops the classifier: one network is trained with and without the condition, and sampling extrapolates from the unconditional prediction towards the conditional one, trading some diversity for samples that fit the condition more strongly. Liu et al.~\cite{liu2022composable} showed that the same arithmetic can combine several conditions, including negated ones, where subtracting the prediction for an unwanted concept moves samples away from it. This work was done on images and text prompts. We haven't found prior work that uses guidance to push tabular samples away from a class in order to help anomaly detectors, and that is the gap our method targets.

\section{Intended approach}
\label{sec:approach}

Figure~\ref{fig:pipeline} shows the generator. It is one pipeline for both datasets, as the rules require, with only the guidance weight $w$, the training length and $N$ set per dataset.
